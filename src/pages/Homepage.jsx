...
export default function Homepage() {
	...
	const refreshToken = async () => {
		try {
			//const token = getCookieValue('refreshToken')
			//console.log(token)
			let response = await axios.get('http://127.0.0.1:3000/auth/token', {
				withCredentials: 'true',
			})
			console.log(response)
			let accessToken = response.data.accessToken
			//setToken(response.data.payload.content)
	...
	return (
		<div>
			...
			<div className="flex flex-col w-full px-6 pb-8 lg:pb-16 lg:flex-row gap-x-3 lg:px-24">
				...
				{/* <div className='flex flex-col w-full gap-2 divide-y-2 lg:px-3 lg:w-1/2'>
          {posts.map((post, index) => {
            return (
              <div key={index}>
                <div className='flex justify-between py-4 '>
                  <CloudTag text={post.category} />
                  <div className='flex items-center gap-1 text-gray-600'>
                    <IconThumbUpFilled size={20} />
                    <span className='text-xs font-medium 2xl:text-sm'>Likes {post.like}</span>
                  </div>
                </div>
                <NavLink to={post.url_img} className='font-semibold text-gray-700 truncate 2xl:text-xl text-md'>
                  <p className='truncate'>{post.title}</p>
                </NavLink>
                <p className='text-xs text-gray-400 2xl:text-sm line-clamp-3'>{post.desc}</p>
                <div className='flex items-center justify-between pt-2'>
                  <div className='flex items-center gap-2'>
                    <Image className={"object-cover 2xl:h-8 h-6 w-6 2xl:w-8 rounded-full"} height={25} width={25} />
                    <div className='flex flex-col'>
                      <p className='text-xs font-semibold text-gray-600 2xl:text-sm'>{post.author}</p>
                      <span className='text-xs text-gray-400'>{post.updated_at}</span>
                    </div>
                  </div>
                </div>
              </div>
			...
			<div className="px-6 py-12 bg-gray-100 lg:px-24">
				...
				<div className="flex w-full gap-8 py-8">
					<div className="flex flex-col w-1/2 gap-6 pb-4 overflow-hidden bg-white rounded-xl">
						<Image
							className={
								'object-cover 2xl:h-96 h-48 lg:h-80 md:h-60 w-full rounded-xl'
							}
						...
						<p className="px-8 text-sm text-gray-400 line-clamp-6">
						...
						<NavLink
							to={'/'}
							className="flex gap-3 px-8 text-gray-500 transition-all duration-300 ease-in-out hover:text-yellow-600">
							Read more
							<IconArrowRight />
						</NavLink>
					</div>
